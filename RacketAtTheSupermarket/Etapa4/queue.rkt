...
(define (enqueue x q)
  (match q
    [(queue left right size-l size-r)
     (if (equal? size-l size-r)
         (struct-copy queue q
                      [left (rotate left (cons x right) empty-stream)]
                      [right '()]
                      [size-l (+ size-l (+ size-r 1))]
                      [size-r 0])
...
(define (dequeue q)
  (match q
      [(queue left right size-l size-r)
       (if (equal? size-l size-r)
           (struct-copy queue q
                        [left (rotate (stream-rest left) right empty-stream)]
                        [right '()]
                        [size-l (+ (- size-l 1) size-r)]
                        [size-r 0])
           (struct-copy queue q [left (stream-rest left)] [size-l (- size-l 1)]))]))
